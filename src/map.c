...
void insert(Map *this, int time, void *value)
{
    if (time > this->size - 1)
    {
        if (quiet == 0) {
            if (this->type == PROCESS_M) {
                ...
                fprintf(output, "Erro: o tempo máximo para I/O é de %d, pulando operação\n", this->size);
        }
        return;
    }
    ...
    if (quiet == 0) {
        if (this->type == PROCESS_M) {
            Process *process = (Process*) value;
            fprintf(output, "Inserindo processo %s em t=%d com duração de %d;\n", process->name, time, process->length);
        }
        else {
            IO *io = (IO*) value;
            fprintf(output, "\tInserindo I/O de %s em t=%d;\n", name(io->type), io->start);
        }
    }
